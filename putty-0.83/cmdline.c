                          ...
                          int need_save, Conf *conf)
{
    ...
    if (!strcmp(p, "-proxyssh") || !strcmp(p, "-J")) {
        char* host, * portp, * user, * temp;
        ...
        portp = host_strchr(value, ':');
        if (!portp) {
            cmdline_error("-proxyssh expects argument of form 'username@host:port'");
            return ret;
        }

        temp = dupprintf("%.*s", (int)(portp - value), value);

        host = host_strchr(temp, '@');
        if (!host) {
            cmdline_error("-proxyssh expects argument of form 'username@host:port'");
            return ret;
        }

        user = dupprintf("%.*s", (int)(host - temp), temp);

        conf_set_int(conf, CONF_proxy_type, PROXY_SSH_TCPIP);
        conf_set_str(conf, CONF_proxy_username, user);
        conf_set_str(conf, CONF_proxy_host, (host + 1));
        conf_set_int(conf, CONF_proxy_port, atoi(portp + 1));
    }
